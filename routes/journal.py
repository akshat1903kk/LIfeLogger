...
@router.get("/{id}")  # ✅ Added endpoint for fetching a single journal entry
def get_journal(id: int, db: db_dependency, user: user_dependency):
    journal_entry = db.query(Journal).filter(Journal.id == id, Journal.user_id == user.get('id')).first()
    
    if not journal_entry:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Journal not found")
    
    return journal_entry  # ✅ Returns the journal entry
